define(['./index', 'angular'], function (services, ng) {
/** shamelessly stolen from https://developer.mozilla.org/en-US/docs/DOM/document.cookie
 **/
function CookieService(config) {
    var api = {
        ...
        /**
        * write(sKey, sValue, vEnd, sPath, sDomain, bSecure)
        *
        ...
        * @return undefined;
        **/
        ,write: function (sKey, sValue, vEnd, sPath, sDomain, bSecure) {
            sDomain = (sDomain || config.COOKIE_DOMAIN);
        ...
        /* comply with angular's cookieStore API
         * deserialize JSON cookie; assumes this was placed with `put`
         * */
        ,get: function (sKey) {
            var data = this.read(sKey);
            return ng.fromJson(data);
        }
        /* comply with angular's cookieStore API
         * serialize cookie value into JSON; assumes this will be fetched with `get`
         * */
        ,put: function (sKey, sValue) {
            var data = ng.toJson(sValue);
            return this.write(sKey, data);
        }
    };
    return api;
};

services.factory('cookies', ['config', CookieService]);
